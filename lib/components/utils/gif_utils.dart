...
class GifAssetImage extends GifAssetBundleImageProvider {
  ...
  static Future<Map<String, List<String>>> _manifestParser(String jsonData) {
    if (jsonData == null) {
      return SynchronousFuture<Map<String, List<String>>>(Map());
    }
  ...
  double _parseScale(String key) {
    ...
    final RegExpMatch? match = _extractRatioRegExp.firstMatch(directoryPath);
...
class GifMultiFrameImageStreamCompleter extends GifImageStreamCompleter {
  ...
  List<ui.FrameInfo>? _frames = <ui.FrameInfo>[];
  ...
  void _handleCodecReady(ui.Codec codec) async {
    ...
    if ((_codec?.frameCount ?? 0) > 1) {
      _frames = <ui.FrameInfo>[];
      for (int i = 0; i < (_codec?.frameCount ?? 0); i++) {
  ...
  void _handleAppFrame(Duration timestamp) {
    ...
    if (_isFirstFrame() || _hasFrameDurationPassed(timestamp)) {
      _emitFrame(ImageInfo(image: _nextFrame!.image, scale: _scale ?? 0));
      _shownTimestamp = timestamp;
      _frameDuration = _nextFrame?.duration;
      _nextFrame = null;
      final int completedCycles = _framesEmitted ~/ (_codec?.frameCount ?? 0);
      // *******Add
      // 自定义循环次数
      // 不使用_codec本身的循环次数
...
//      }
  ...
  Future<void> _decodeNextFrameAndSchedule() async {
    try {
      if ((_frames?.length ?? 0) > 0) {
        int frameNum = _framesEmitted % (_codec?.frameCount ?? 0);
        if (reverse) {
          _nextFrame = _frames![(_codec?.frameCount ?? 0) - frameNum - 1];
          print("<gifImage> frameCount ${(_codec?.frameCount ?? 0) - frameNum - 1}");
        } else {
  ...
  void addListener(ImageStreamListener listener) {
    if (!hasListeners) _decodeNextFrameAndSchedule();
